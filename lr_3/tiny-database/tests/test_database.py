...
import os
import tempfile
from database.database import Database, EmployeeTable, DepartmentTable, SalesTable
...
def test_department(database):
    ...
    assert department_data[0] == {
        'id': '2', 
        'department_name': 'Finance'
    }
    assert department_data[1] == {
        'id': '5', 
        'department_name': 'Finance'
    }
...
def test_read_department(database, temp_department_file):
    ...
    assert department_data[2] == {
        'id': '3', 
        'department_name': 'Marketing'
    }
    assert department_data[3] == {
        'id': '4', 
        'department_name': 'IT'
    }
    assert department_data[4] == {
        'id': '5', 
        'department_name': 'Finance'
    }
...
def test_double_join_with_aggregate(database):
    ...
    with pytest.raises(ValueError) as excinfo:
        aggregate_data = database.aggregate("test", "price", double_joined_data) #такой агрегации нет
...
def test_aggregation(database):
    ...
    test_table = [
        {"name" : "Test",
        "info" : "This is test string"},
        {"name" : "Second",
        "info" : "This is second test string"}
    ]
    with pytest.raises(ValueError) as excinfo:
        aggregate_data = database.aggregate("avg", "price", test_table)
    ...
    assert str(excinfo.value) == "Can't aggregate non-numeric value(-s) with average method."
